...
void scrub_buffer(void * raw_sk){
    int i;
    for(i =0; i < 2*CCA_STRENGTH; i++){
        *( (unsigned char*)(raw_sk) + i)=0;
    }
    return;
}
void inc_counter(char* ctr){
  /* Should add 1 to the integer stored in ctr */
  /* YOUR CODE HERE */
  int * int_array = (int* ) ctr;
  int num_ints = CCA_STRENGTH / sizeof(int);
  int curr_space = num_ints-1;
  unsigned int max_int_u = (((unsigned int) (1)) << ( ( 8 * sizeof(int)) - 1)) -1;
  int max_int = (int) max_int_u;;
  while(int_array[curr_space] + 1 >= max_int){
      int_array[curr_space] = 0;
      curr_space++;
  }
  int_array[curr_space] +=1; 
  return; 
}
void ri (void){
